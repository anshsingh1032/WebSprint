...
import { addKarma } from '../store/authSlice';
 ...
 if (loading) return (
  ...
  <form onSubmit={handleReplySubmit}>
    <textarea
      ...
      className="w-full px-5 py-4 bg-white rounded-xl border border-gray-200 focus:outline-none focus:ring-4 focus:ring-violet-500/20 focus:border-violet-500 transition-all font-medium resize-none mb-2 shadow-sm"
    ></textarea>

    <div className="flex flex-col md:flex-row justify-between items-center gap-4">
      {/* --- NEW FILE INPUT --- */}
      <div className="flex items-center gap-2">
        <label className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 transition-all text-sm font-bold text-gray-600">
          <span className="text-lg">📎</span>
          {replyFile ? replyFile.name : "Attach Image/File"}
          <input 
            type="file" 
            className="hidden" 
      ...
      <button
        ...
        className="w-full md:w-auto bg-gray-900 hover:bg-violet-600 text-white font-bold py-3 px-8 rounded-xl transition-all shadow-md hover:shadow-lg disabled:opacity-70"
      >
